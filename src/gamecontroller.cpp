...
int GameController::winner() const {
    ...
    if (userKalah > jinnKalah) return 0; // USER wins
    if (jinnKalah > userKalah) return 1; // JINN wins
    return -1;                            // tie
}
...
void GameController::sow(int pitIndex) {
    if (m_aiThinking || gameOver()) return;
    if (m_game->getPosition().currentPlayer != Player::USER) return;
